...
// .wrangler/tmp/pages-aFjIOT/bundledWorker-0.9068705890934554.mjs
...
var q = /* @__PURE__ */ __name2((e, t, r) => (r = e != null ? re(oe(e)) : {}, ce(t || !e || !e.__esModule ? H(r, "default", { value: e, enumerable: true }) : r, e)), "q");
var x;
var u = k(() => {
  x = { collectedLocales: [] };
});
...
__name(R, "R");
__name2(R, "R");
function A(e, { url: t, cookies: r, headers: s, routeDest: a }) {
  switch (e.type) {
    case "host":
...
async function j(e, { request: t, assetsFetcher: r, ctx: s }, { path: a, searchParams: o }) {
  ...
  try {
    switch (e?.type) {
      ...
      case "override": {
        n = N(await r.fetch(I(c, e.path ?? a))), e.headers && m(n.headers, e.headers);
        break;
      }
      case "static": {
...
function he(e, t) {
  ...
  if (!d && !t?.domains?.includes(i.hostname) && !t?.remotePatterns?.find((T) => de(i, T))) return;
...
async function Z(e) {
  ...
  try {
    ...
    switch (e.method) {
      case "GET": {
        let o = W(e, xe), n = await s.get(a, { softTags: o });
...
function ye() {
  ...
  globalThis.fetch = async (...t) => {
    ...
    return s || (s = await Z(r), s) ? s : (Te(r), e(r));
  };
}
__name(ye, "ye");
__name2(ye, "ye");
...
var E = class {
  ...
  checkRouteMatch(t, { checkStatus: r, checkIntercept: s }) {
    ...
    }) && !t.missing?.find((n) => A(n, o).valid) && !(r && t.status !== this.status)) {
      if (s && t.dest) {
  ...
  applyLocaleRedirects(t) {
    ...
    let { locale: { redirect: s, cookie: a } } = t, o = a && this.cookies[a], n = L(o ?? ""), i = L(this.reqCtx.request.headers.get("accept-language") ?? ""), h = [...n, ...i].map((g) => s[g]).filter(Boolean)[0];
    if (h) {
...
var Ns = { async fetch(e, t, r) {
  ...
  return a.run({ ...t, NODE_ENV: "production", SUSPENSE_CACHE_URL: w }, async () => o.run({ env: t, ctx: r, cf: e.cf }, async () => {
    if (new URL(e.url).pathname.startsWith("/_next/image")) return K(e, { buildOutput: f, assetsFetcher: t.ASSETS, imagesConfig: _.images });
    let i = G(e);
    return ee({ request: i, ctx: r, assetsFetcher: t.ASSETS }, _, f, x);
  }));
} };
...
// .wrangler/tmp/pages-aFjIOT/6svt9l47df.js
...
// .wrangler/tmp/bundle-PQIwMX/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-PQIwMX/middleware-loader.entry.ts
 ...
 * MIT Licensed
 */
//# sourceMappingURL=6svt9l47df.js.map
