      ...
      <DialogContent className="max-w-4xl p-0 overflow-hidden border-none rounded-[32px] shadow-2xl flex flex-col h-[85vh]">
        ...
        <DialogHeader className="p-8 bg-gray-50 dark:bg-secondary border-b border-gray-100 dark:border-border shrink-0">
          <div className="flex items-center gap-3 mb-6">
            ...
            <div>
              <DialogTitle className="text-2xl font-black text-[#222222] dark:text-zinc-100 tracking-tight">クレジットと法務情報</DialogTitle>
              <DialogDescription className="text-gray-500 dark:text-zinc-400 font-medium">Oshi-Linkの運営・技術・規約について</DialogDescription>
            </div>
          </div>
          ...
          {activeTab === 'credits' && (
            <div className="space-y-10 animate-in fade-in slide-in-from-bottom-2 duration-300">
              <section className="space-y-4">
                <h3 className="flex items-center gap-2 text-[11px] font-black text-gray-400 dark:text-zinc-500 uppercase tracking-widest">
                  <Info className="w-4 h-4" /> Concept & Vision
                </h3>
                <div className="p-6 rounded-[24px] bg-gradient-to-br from-[#6366f1]/5 to-[#6366f1]/10 dark:from-[#6366f1]/10 dark:to-[#6366f1]/20 border border-[#6366f1]/10 dark:border-[#6366f1]/20">
                  <p className="text-[14px] font-black text-[#222222] dark:text-zinc-100 mb-2">推し活の「今」を、みんなで形にする。</p>
                  <p className="text-[12px] text-gray-600 dark:text-zinc-300 leading-relaxed font-medium">
        ...
        <div className="p-6 border-t border-gray-100 dark:border-border bg-gray-50 dark:bg-secondary shrink-0 text-center">
          ...
          <p className="text-[10px] text-gray-300 dark:text-zinc-650 font-medium">© 2026 TSUKURO! All rights reserved.</p>
        </div>
      </DialogContent>
    </Dialog>
  );
...
function TermSection({ title, children }: { title: string; children: React.ReactNode }) {
  return (
    <div className="space-y-2">
      <h4 className="text-[14px] font-black text-[#222222] dark:text-zinc-100">{title}</h4>
      <div className="text-[13px] text-gray-600 dark:text-zinc-300 leading-relaxed font-medium">
        {children}
      </div>
    </div>
  );
