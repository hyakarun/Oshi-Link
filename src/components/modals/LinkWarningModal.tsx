...
export function LinkWarningModal({ url, onClose }: LinkWarningModalProps) {
  return (
    <Dialog open={!!url} onOpenChange={(open) => !open && onClose()}>
      <DialogContent className="max-w-md p-8 border-none rounded-[32px] bg-white dark:bg-zinc-900 shadow-2xl">
        <div className="flex flex-col items-center text-center space-y-4">
          ...
          <p className="text-sm font-medium text-gray-550 dark:text-zinc-400 bg-gray-50 dark:bg-zinc-800 p-4 rounded-xl break-all w-full text-left max-h-[100px] overflow-y-auto">
            {url}
          </p>
