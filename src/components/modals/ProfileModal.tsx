...
import { Bell, Mail, Info, Sun, Moon, Laptop, Palette, BookOpen } from 'lucide-react';
...
export function ProfileModal({
  ...
  const [activeTab, setActiveTab] = React.useState<'settings' | 'info'>('settings');
    ...
    <Dialog open={isOpen} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-md p-0 overflow-hidden rounded-3xl border-none shadow-2xl bg-white dark:bg-zinc-900 bottom-0 sm:bottom-auto top-auto sm:top-1/2 translate-y-0 sm:-translate-y-1/2 transition-all duration-500">
        <div className="p-8 space-y-6 max-h-[85vh] overflow-y-auto">
          ...
          {/* タブ切り替え */}
          <div className="flex border-b border-gray-100 dark:border-zinc-800 -mx-8 px-8">
            <button
              type="button"
              onClick={() => setActiveTab('settings')}
              className={`flex-1 pb-3 text-[11px] font-black tracking-wider uppercase transition-all border-b-2 cursor-pointer ${
                activeTab === 'settings'
                  ? 'border-[#6366f1] text-[#6366f1]'
                  : 'border-transparent text-gray-400 dark:text-zinc-500 hover:text-gray-650 dark:hover:text-zinc-300'
              }`}
            >
              設定
            </button>
